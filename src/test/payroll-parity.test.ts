/**
 * SomaCampus Phase 7: Payroll Reference-Parity Fixtures
 *
 * Reference parity fixtures ported from the JantaHR reference implementation
 * (no runtime dependency). Each case: reference / input / expected.
 *
 * Covers (inputs + expectations only; JantaHR is reference-only):
...
describe('Uganda Statutory Payroll Reference-Parity Fixtures', () => {
  describe('PAYE Tax Bands (Income Tax (Amendment) Act, 2026)', () => {
    it('[PAYE_NIL] charges nil tax at or below the 335,000 threshold', () => {
    ...
    it('[PAYE_10PCT] charges 10% on excess over 335,000 up to 410,000', () => {
    ...
    it('[PAYE_25PCT] charges 7,500 + 25% on excess over 410,000 up to 485,000', () => {
    ...
    it('[PAYE_30PCT] charges 26,250 + 30% on excess over 485,000', () => {
    ...
    it('[PAYE_CONTINUITY] ensures bands are continuous with no jump at boundaries', () => {
    ...
    it('[PAYE_MONOTONIC] ensures tax is monotonic — higher gross never pays less tax', () => {
      let prev = -1;
    ...
    it('[PAYE_15M] [PAYE_SURCHARGE] applies 10% super-earner surcharge above 10,000,000 threshold', () => {
    ...
    it('[PAYE_CUSTOM_BANDS] respects custom-configured tax bands', () => {
  ...
  describe('NSSF Contributions (NSSF Act Cap 222)', () => {
    it('[NSSF_STANDARD] deducts 5% employee, 10% employer, 15% total with no wage ceiling', () => {
    ...
    it('[NSSF_RATIO] ensures employer contribution is always double employee share', () => {
    ...
    it('[NSSF_ROUNDING] rounds to whole shillings and total equals the parts', () => {
  ...
  describe('Overtime Calculations', () => {
    it('[OVERTIME_ZERO] returns zero for missing or non-positive hours', () => {
    ...
    it('[OVERTIME_1_5X] calculates 1.5x hourly rate over 173.33 standard monthly hours', () => {
    ...
    it('[OVERTIME_CUSTOM_RATE] respects custom flat overtime rate', () => {
  ...
  describe('Worker Tax Classifications', () => {
    it('[LOCAL_PAYE_NSSF] taxes local employee with PAYE and NSSF', () => {
    ...
    it('[GLOBAL_PAYE_ONLY] taxes global employee with PAYE only (no NSSF)', () => {
    ...
    it('[CONTRACTOR_WHT] taxes contractor with WHT only (default 6%)', () => {
    ...
    it('[CONTRACTOR_WHT_OVERRIDE] allows contractor WHT override rate', () => {
      const p = calculateUgandaPayslip({ grossSalary: 1_000_000, employeeType: 'contractor', whtRate: 10 });
      expect(p.whtAmount).toBe(100_000);
    });

    it('[EXEMPT_NIL] does not withhold anything for exempt employees', () => {
  ...
  describe('Single-Writer Item Composer (buildPayrollItem)', () => {
    it('[COMPOSER_RECONCILE] reconciles: gross earnings - deductions === net pay', () => {
      const cases = [
    ...
    it('[SALARY_ADVANCE] [UNPAID_LEAVE] correctly applies advance deductions and unpaid leave deductions', () => {
    ...
    it('[NETPAY_CLAMP_ZERO] clamps net pay at zero when deductions exceed earnings', () => {
      const item = buildPayrollItem({
        grossSalary: 400_000,
  ...
  describe('Formatting Utility (formatUGX)', () => {
    it('[FORMAT_UGX] formats whole shillings with thousands separator', () => {
